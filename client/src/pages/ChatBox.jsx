...
const ChatBox = () => {
  ...
  return user &&(
    <div className='flex flex-col h-screen'>
      <div className='flex items-center gap-2 p-2 md:px-10 xl:pl-42 bg-linear-to-r from-green-50 to-green-100 border-b border-gray-300'>
        ...
        <div className='space-y-4 max-w-4xl mx-auto'>
          {
            messages.toSorted((a, b) => new Date(a.createdAt) - new Date(b.createdAt)).map((message, i) => (
              <div key={i} className={`flex flex-col ${message.to_user_id !== user._id ? 'items-start' : 
                'items-end'
              }`}>
      ...
      <div className='px-4'>
        <div className='flex items-center gap-3 pl-5 pr-3 p-1.5 bg-white w-full max-w-xl mx-auto border border-gray-200 shadow rounded-full mb-5'>
          ...
          <button onClick={sendMessage} className='bg-linear-to-br from-green-500 to-green-600 hover:from-green-700 hover:to-green-800 active:scale-95 cursor-pointer text-white p-2 rounded-full'>
