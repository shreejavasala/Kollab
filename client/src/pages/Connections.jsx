  ...
  return (
    <div className='min-h-screen bg-slate-50'>
      <div className='max-w-6xl mx-auto p-6'>
        ...
        {/* Title */}
        <div className='mb-8'>
          <h1 className='text-3xl font-bold tex-slate-900 mb-2'>Connections</h1>
        ...
        {/* Tabs */}
        <div className='inline-flex flex-wrap items-center justify-between border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
          {
            dataArray.map((tab) => (
              <button onClick={() => setCurrentTab(tab.label)} key={tab.label} className={`flex items-center px-3 py-1 text-sm rounded-md cursor-pointer transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black' : 'text-gray-500 hover:text-black'}`}>
                ...
                {tab.count !== undefined && (
                  <span className='ml-2 text-sm bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full'>{tab.count}</span>
                )}
              </button>
            ))
          }
            ...
            <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
              ...
              <div className='flex-1'>
                ...
                <div className='flex max-sm:flex-col gap-2 mt-4'>
                  {
                    <button onClick={() => navigate(`/profile/${user._id}`)} className='w-full p-2 text-sm rounded bg-linear-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 active:scale-95 transition text-white cursor-pointer'>
                      View Profile
                    </button>
                  }
                  {
